...
- Historical blocker note: the full reference corpus was cloned under `references/` (git-ignored, LFS-skipped) to drive planning. The current repo now has committed fixture gates for the completed phases and Phase 1 env-gated parity checks for LocateAnything, RF-DETR detector, and SAM 3.1 image-mode; `.agent/work/2026-06-16-release-parity-hardening/parity-status.json` currently records upstream parity as blocked until the required checkpoints and stable reference tap paths are available. (`§16.6`)
