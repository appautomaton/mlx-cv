...
**Status:** complete
**Evidence:** updated `src/mlx_cv/parity/fixtures.py` / `src/mlx_cv/parity/__init__.py`, `tools/mint_rfdetr_fixture.py`, `src/mlx_cv/heads/detection/rfdetr.py`, and `src/mlx_cv/models/rfdetr/modeling.py`; added committed `tests/fixtures/rfdetr_tiny_fixture.npz` and `tests/fixtures/rfdetr_tiny_fixture_weights.npz`, plus `tests/test_rfdetr_parity.py` and `tests/test_rfdetr_predict.py`; `uv run pytest tests/test_rfdetr_parity.py tests/test_rfdetr_predict.py` passed with 6 tests, and the broader RF-DETR/dependency guard run passed with 31 tests.
**Risks / next:** committed RF-DETR detector fixture is generated by the local MLX tiny oracle in this workspace; the mint tool still supports the existing RF-DETR reference deformable-attention fixture and optional reference postprocess in a throwaway Torch environment.
...
**Status:** complete
**Evidence:** added SAM3 fixture registry entries, model capture taps, `tools/mint_sam3_fixture.py`, committed `tests/fixtures/sam3_tiny_fixture_{text,pcs}.npz` and `tests/fixtures/sam3_tiny_fixture_weights.npz`, plus `tests/test_sam3_{parity,predict}.py`; `uv run python tools/mint_sam3_fixture.py` minted the committed fixtures, `uv run pytest tests/test_sam3_parity.py tests/test_sam3_predict.py` passed with 9 tests, and the broader SAM3/dependency guard run passed with 46 tests.
**Risks / next:** committed SAM3 fixtures are generated by a local MLX image-mode oracle because this workspace has no SAM3 reference checkout with stable tap points; fixture metadata records submodule-level local tap points. Final status/docs must report this boundary truthfully in Slice 12.
