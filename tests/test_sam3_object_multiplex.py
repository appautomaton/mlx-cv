...
def test_sam3_object_multiplex_track_metadata_matches_bucket_state():
    ...
    state = manager.start_session(frames=_frames())
    manager.add_prompt(state.session_id, prompt=BoxPrompt([[1, 1, 4, 4]]), object_id=10)
    manager.add_prompt(state.session_id, prompt=BoxPrompt([[2, 2, 5, 5]]), object_id=11)
    ...
    for frame in video.frames:
        ...
        assert frame.tracks.metadata == [
            {"prompt_mode": "sam3_tracker", "multiplex_bucket": 0},
            {"prompt_mode": "sam3_tracker", "multiplex_bucket": 0},
        ]
...
def test_sam3_object_multiplex_updates_when_objects_are_removed_and_added():
    ...
    manager.add_prompt(state.session_id, prompt=BoxPrompt([[3, 3, 6, 6]]), object_id=3)
