...
class SAM3CheckpointError(ValueError):
    ...
    try:
        with path.open("rb") as handle:
            ...
            if header_size <= 0 or header_size > 100_000_000:
                raise SAM3CheckpointError(f"invalid Safetensors header size: {header_size}")
            header = json.loads(handle.read(header_size))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise SAM3CheckpointError(f"cannot read SAM 3.1 Safetensors header: {path}") from exc
    metadata = header.get("__metadata__", {})
    if not isinstance(metadata, dict) or not all(
        isinstance(key, str) and isinstance(value, str)
        for key, value in metadata.items()
    ):
        raise SAM3CheckpointError("SAM 3.1 Safetensors metadata must be string pairs")
    return metadata
...
def _validate_metadata(metadata: dict[str, str]) -> None:
    errors = {
        ...
        for key, expected in SAM3_CHECKPOINT_METADATA.items()
    ...
    if errors:
        ...
        raise SAM3CheckpointError(f"incompatible SAM 3.1 checkpoint metadata: {details}")


def _read_weights(path: str | Path) -> tuple[dict[str, mx.array], dict[str, str]]:
    path = Path(path)
    if path.suffix != ".safetensors":
        raise SAM3CheckpointError("SAM 3.1 production weights must use .safetensors")
    metadata = read_safetensors_metadata(path)
    _validate_metadata(metadata)
    weights = mx.load(str(path))
    if not isinstance(weights, dict):
        raise SAM3CheckpointError("SAM 3.1 Safetensors did not contain a tensor map")
...
def load_sam3_weights(model: Any, path: str | Path) -> Any:
    ...
    if metadata.get("scope") == "multiplex":
        ...
        raise SAM3CheckpointError("SAM 3.1 checkpoint has no detector scope")
    return _load_weights(model, weights)


def load_sam3_tracker_weights(model: Any, path: str | Path) -> Any:
    """Load the tracker subtree from the final combined BF16 checkpoint."""

    weights, metadata = _read_weights(path)
    if metadata.get("scope") != "multiplex":
        raise SAM3CheckpointError("SAM 3.1 checkpoint has no tracker scope")
    prefix = "tracker."
    selected = {
        key.removeprefix(prefix): value
        for key, value in weights.items()
...
def load_sam3_video_weights(model: Any, path: str | Path) -> Any:
    """Load the complete final-layout detector + tracker checkpoint."""

    weights, metadata = _read_weights(path)
    if metadata.get("scope") != "multiplex":
        raise SAM3CheckpointError("complete SAM 3.1 load requires multiplex scope")
    return _load_weights(model, weights)
